...
const commands: Record<string, string> = {
  ...
  about: "Prashanth. AI & systems engineer. Coimbatore. Builds ML pipelines, compilers, and the infrastructure in between.",
  work: "Medclara (founding eng) · primesoma (founder) · self (founder) · LLVM contributor · EfficientXLang (research).",
  stack: "Go · Python · TypeScript · C++ · Next.js · Whisper · LLVM · UE5 · DaVinci Resolve",
  ...
  return (
    <>
      <button
        ...
        className="fixed bottom-5 right-5 z-[55] font-mono text-[10px] tracking-[0.2em] uppercase text-white/40 hover:text-white border border-white/10 hover:border-white/30 bg-white/5 backdrop-blur-md rounded-md px-3 py-2 transition-colors"
        title="Open terminal (⌘`)"
      >
        &gt;_ term
      </button>
        ...
        {open && (
          ...
          >
            <div className="flex items-center justify-between px-4 py-2 border-b border-white/10 bg-white/[0.02]">
              ...
              <span className="text-white/30 text-[10px] tracking-[0.2em] uppercase">starone01 — tty1</span>
              <button onClick={() => setOpen(false)} className="text-white/30 hover:text-white text-xs">esc</button>
            </div>
            ...
            <div ref={scrollRef} className="flex-1 overflow-y-auto p-4 space-y-1">
              {history.map((line, i) => (
                <div key={i} className={cn(
                  ...
                  line.kind === "out" && "text-white/70",
                  line.kind === "err" && "text-white/30 italic"
                )}>
                  {line.kind === "in" ? (
                    <>
                      <span className="text-white/30">guest@starone01</span>
