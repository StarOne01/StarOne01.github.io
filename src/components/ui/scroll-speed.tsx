...
export default function ScrollSpeed() {
  ...
  return (
    <div
      className={`fixed bottom-20 max-sm:bottom-36 right-4 z-50 transition-all duration-500 ${
        visible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4 pointer-events-none"
      }`}
    >
      <div className="bg-carbon border border-white/12 rounded-lg px-4 py-2 text-sm text-white/60 whitespace-nowrap font-mono">
        <span className="text-volt">{speed}px/s</span> — {msg}
      </div>
    </div>
  );
}
